...
Renderer::Renderer(Window& parent) : OGLRenderer(parent) {
	...
	camera = new Camera(-45, 0, heightMapSize * Vector3(0.5f, 5, 0.5f));

	pointLights = new Light[LIGHT_NUM];
	for (int i{}; i < LIGHT_NUM; ++i) {
		Light& l = pointLights[i];
		l.SetPosition(Vector3(rand() % static_cast<int>(heightMapSize.x), 150, rand() % static_cast<int>(heightMapSize.z)));
		l.SetColour(Vector4(0.5f + static_cast<float>(rand() / static_cast<float>(RAND_MAX)), 
								0.5f + static_cast<float>(rand() / static_cast<float>(RAND_MAX)), 
								0.5f + static_cast<float>(rand() / static_cast<float>(RAND_MAX)), 1));
...
void Renderer::FillBuffers() {
	...
	projMatrix = Matrix4::Perspective(1, 10000, static_cast<float>(width) / static_cast<float>(height), 45);
