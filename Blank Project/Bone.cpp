...
Bone::Bone(std::string name, const int ID, const aiNodeAnim* channel) : name(std::move(name)), ID(ID) {
	...
	for (int rotIndex = 0; rotIndex < numRotations; ++rotIndex) {
		KeyRotation data;
		data.orientation = channel->mRotationKeys[rotIndex].mValue;
	...
	for (int scaleIndex = 0; scaleIndex < numScales; ++scaleIndex) {
		KeyScale data;
		data.scale = channel->mScalingKeys[scaleIndex].mValue;
		data.timeStamp = channel->mScalingKeys[scaleIndex].mTime;
		scales.push_back(data);
	}

	restingRotation = rotations[0].orientation;
}
...
Matrix4 Bone::InterpolatePosition(const float anim_dt) {
	if (numPositions == 1) return Matrix4::Translation(Vector3(positions[0].position.x, positions[0].position.y, positions[0].position.z));
	...
	aiVector3D finalPos = positions[posIndex0].position * (1 - scaleFactor) + positions[posIndex1].position * scaleFactor;

	return Matrix4::Translation(Vector3(finalPos.x, finalPos.y, finalPos.z));
}
...
Matrix4 Bone::InterpolateRotation(const float anim_dt) {
	if (numRotations == 1) {
		...
		rot.Normalize();
		return AssimpNCLHelpers::GetNCLMatrix(rot);
	...
	aiQuaternion::Interpolate(finalRot, rotations[rotIndex0].orientation, rotations[rotIndex1].orientation, scaleFactor);
	finalRot.Normalize();
	return AssimpNCLHelpers::GetNCLMatrix(finalRot);
...
Matrix4 Bone::InterpolateScale(const float anim_dt) {
	if (numScales == 1) return Matrix4::Scale(Vector3(scales[0].scale.x, scales[0].scale.y, scales[0].scale.z));
	...
	aiVector3D finalScale = scales[scaleIndex0].scale * (1 - scaleFactor) + scales[scaleIndex1].scale * scaleFactor;

	return Matrix4::Scale(Vector3(finalScale.x, finalScale.y, finalScale.z));
}