...
Renderer::Renderer(Window& parent) : OGLRenderer(parent) {
	...
	positions[0] = Vector3(0, 0, -5); // 5 units from viewpoint
	...
	projMatrix = Matrix4::Perspective(1, 100, static_cast<float>(width) / static_cast<float>(height), 45);
...
void Renderer::ToggleObject() { modifyObject = !modifyObject; }

void Renderer::MoveObject(float by) { positions[(int)modifyObject].z += by; }
