...
Renderer::Renderer(Window& parent) : OGLRenderer(parent) {
	camera = new Camera(-30, 315, Vector3(-8, 5, 8));
	light = new Light(Vector3(-20, 10, -20), Vector4(1, 1, 1, 1), 250);
...
void Renderer::UpdateScene(float dt) {
	...
	for (int i = 1; i < 4; ++i) {
		Vector3 t = Vector3(-10 + 5 * i, 2 + sin(sceneTime * i), 0);
...
void Renderer::DrawMainScene() {
	...
	projMatrix = Matrix4::Perspective(1, 15000, static_cast<float>(width) / static_cast<float>(height), 45);
	auto camPos = camera->GetPosition();

	glUniform1i(glGetUniformLocation(sceneShader->GetProgram(), "diffuseTex"), 0);
	glUniform1i(glGetUniformLocation(sceneShader->GetProgram(), "bumpTex"), 1);
	glUniform1i(glGetUniformLocation(sceneShader->GetProgram(), "shadowTex"), 2);
	glUniform3fv(glGetUniformLocation(sceneShader->GetProgram(), "cameraPos"), 1, (float*)&camPos);
